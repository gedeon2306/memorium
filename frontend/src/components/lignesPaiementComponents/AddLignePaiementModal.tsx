...
import { createLignePaiement } from "@/app/actions/actions";
import { motion } from "framer-motion";
import { CreditCard, X, Loader2, Plus } from "lucide-react";
...
interface AddLignePaiementModalProps {
  onSuccess?: () => void;
  paiementId?: string;
  defunts?: any[];
}

export interface AddLignePaiementModalHandle {
  open: () => void;
}

const AddLignePaiementModal = forwardRef<AddLignePaiementModalHandle, AddLignePaiementModalProps>(
  ({ onSuccess, paiementId, defunts = [] }, ref) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        const res = await createLignePaiement(payload);
        ...
        const message =
          res && typeof res === "object" && "message" in res && typeof res.message === "string"
            ? res.message
            : "Ligne de paiement ajoutée avec succès.";
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-white/35 mt-0.5">
                  Enregistrer une nouvelle ligne de paiement dans Memorium
                </p>
              </div>
            </div>
            <button
...
AddLignePaiementModal.displayName = "AddLignePaiementModal";

export default AddLignePaiementModal;
