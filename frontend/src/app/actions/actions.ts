...
export async function getDefuntsList(page: number = 1, search: string = "", ordering: string = "nom-asc") {
  const token = await getToken();

  const params = new URLSearchParams({ page: String(page) });
  if (search) params.append("search", search);
  if (ordering) params.append("ordering", ordering);
  const url = `dashboard/defunts/?${params.toString()}`;
...
export async function createDefunt(payload: {
  nom: string;
  prenom?: string;
  genre: string;
  age: number;
...
export async function updateDefunt(payload: {
  id: string;
  nom: string;
  prenom?: string;
  genre: string;
...
export async function deleteDefunt(payload: { id: string }): Promise<DeleteEntityResult> {
  const token = await getToken();
  const raw = process.env.API_URL || process.env.NEXT_PUBLIC_API_URL || "";
  const base = raw.replace(/\/+$/, "");
  const url = `${base}/api/dashboard/defunts/`;
...
export async function getPaiementsList(page: number = 1, search: string = "", ordering: string = "recent") {
  ...
  if (search) params.append("search", search);
  if (ordering) params.append("ordering", ordering);
  const url = `dashboard/paiements/?${params.toString()}`;
...
export async function updatePaiement(payload: {
  id: string;
  famille?: string;
  total_amount?: string;
}) {
...
export async function deletePaiement(payload: { id: string }): Promise<DeleteEntityResult> {
  const token = await getToken();
  const raw = process.env.API_URL || process.env.NEXT_PUBLIC_API_URL || "";
  const base = raw.replace(/\/+$/, "");
  const url = `${base}/api/dashboard/paiements/`;
